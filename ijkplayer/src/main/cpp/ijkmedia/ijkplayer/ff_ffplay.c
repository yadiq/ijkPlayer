    ...
    for (;;) {
        ...
        if (pkt.data == flush_pkt.data) {
            ...
            if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                ...
                //J4A_ALOGE("软解接收packet pts=%d dts=%d", pkt.pts, pkt.dts);
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            //有缓存时，立即播放
            //ALOGE("开始解码，缓存大小 %d", q->nb_packets);
            ...
            //音视频同步逻辑 https://www.cnblogs.com/baitongtong/p/12758622.html
            last_duration = vp_duration(is, lastvp, vp);//上一帧的持续显示时间
            delay = compute_target_delay(ffp, last_duration, is);//当前帧需要等待的时间

            time= av_gettime_relative()/1000000.0;
            if (isnan(is->frame_timer) || time < is->frame_timer)
                is->frame_timer = time;
         
            //frame_timer 上一帧的播放时间
            //frame_timer + delay 是当前这一帧的播放时间
            //如果系统时间还没有到当前这一帧的播放时间，直接跳转至display 睡眠等待
            ...
            if (delay > 0 && time - is->frame_timer > AV_SYNC_THRESHOLD_MAX) //TODO 丢帧不生效的问题
                is->frame_timer = time;
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (is->is_video_high_fps) {//above 30fps
...
static int read_thread(void *arg)
{
    ...
    if (ffp->find_stream_info) {//探测。读取并解码流，用启发式方法填充缺失的信息
    ...
    } else {//不探测时，补全视频宽高比 640:360 (必须大于320:180) TODO
        int orig_nb_streams = ic->nb_streams;
        if (orig_nb_streams > 0) {
            for (i = 0; i < orig_nb_streams; i++) {
                if (!ic->streams[i] || !ic->streams[i]->codecpar || ic->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
                    ic->streams[0]->codecpar->width = 640;
                    ic->streams[0]->codecpar->height = 360;
                    break;
                }
            }
        }
    ...
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0) {
        ...
        ffp->av_sync_type = AV_SYNC_VIDEO_MASTER;//直播。不应该音视频同步，始终播放最新的
...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
    ...
    int avformat_alloc_output_context2_result = avformat_alloc_output_context2(&ffp->m_ofmt_ctx, NULL, NULL, file_name);
    if (avformat_alloc_output_context2_result < 0) {
        char errbuf[1024] = "";
        av_strerror(avformat_alloc_output_context2_result, errbuf, 1024);
        J4A_ALOGE("创建输出的AVFormatContext结构体失败 %d %s", avformat_alloc_output_context2_result, errbuf);
    }

    if (!ffp->m_ofmt_ctx) {
        av_log(ffp, AV_LOG_ERROR, "Could not create output context filename is %s\n", file_name);
        J4A_ALOGE("创建输出文件失败-->%s",file_name);
        goto end;
    }
    ...
    if (pkt_index == ffp->videoindex || pkt_index == ffp->audeoindex) {
        ...
        if (ffp->is_record) {
            ...
            if (0 == av_packet_ref(pkt, packet)) {
                ...
                //J4A_ALOGE("录制packet数量=%d pts=%d dts=%d", ffp->record_count, pkt->pts, pkt->dts);
                ...
                if ((ret = av_interleaved_write_frame(ffp->m_ofmt_ctx, pkt)) < 0) {
                    ...
                    J4A_ALOGE("写入一个AVPacket到输出文件失败 %d %s", ret, errbuf);
                }
...
int save_png(AVFrame *picture, const char *out_file) {
    //媒体文件上下文。表示媒体格式的上下文信息，包括音频、视频、字幕等流的相关信息和元数据
