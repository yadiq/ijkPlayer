...
public class IjkMediaCodecInfo {
    ...
    public static IjkMediaCodecInfo setupCandidate(MediaCodecInfo codecInfo, String mimeType) {
        ...
        if (TextUtils.isEmpty(name))
            return null;
        //打印解码器
        Log.d(TAG, String.format(Locale.US, "解码器: %s", name));

        name = name.toLowerCase(Locale.US);
        int rank = RANK_NO_SENSE;

        if (name.startsWith("c2.") && !name.startsWith("c2.rk.")) {//非标准
            //c2.rk.hevc.decoder 硬解
            //c2.android.hevc.decoder 软解
            //c2.goldfish.hevc.decoder 软解
            rank = RANK_NON_STANDARD;
        } else if (!name.startsWith("omx.")) {//非标准
            rank = RANK_NON_STANDARD;
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2)
                ...
                rank = RANK_TESTED;
        } else if (name.contains(".secure")) {//OMX.Nvidia.h264.decode.secure
            rank = RANK_SECURE;
        } else {
