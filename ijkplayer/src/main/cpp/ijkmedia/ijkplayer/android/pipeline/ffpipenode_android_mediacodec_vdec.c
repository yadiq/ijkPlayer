...
static int feed_input_buffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (d->pkt_temp.data) {
        ...
        if (input_buffer_index < 0) {
            ...
            //J4A_ALOGE("硬解接收packet pts=%d dts=%d size=%d", d->pkt_temp.pts, d->pkt_temp.dts, d->pkt_temp.size);
            //修改ffmpeg源码，减少FFmpeg拆帧等待延时，加快了一帧。
            //解RTP协议后数据长度=RTP长度-12+4
            ...
            if (!ffp->is_first) {
                ffp->is_first = 1;
                ffp->start_pts = d->pkt_temp.pts;
                ffp->start_dts = d->pkt_temp.dts;
            }
            if (ffp->is_record && ffp->is_first) { // 可以录制时，写入文件
                //J4A_ALOGE("硬解码。录制");
                if (0 != ffp_record_file(ffp, &d->pkt_temp)) {
                    ffp->record_error = 1;
                    J4A_ALOGE("意外停止录制");
                    ffp_stop_record(ffp);
                }
...
static int func_run_sync(IJKFF_Pipenode *node)
{
    ...
    //启动线程。解码器写入数据，消耗缓冲区q->nb_packets--
        ...
        if (got_frame) {
            ...
            if (ffp->framedrop > 0 || (ffp->framedrop && ffp_get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) {
                ...
                if (frame->pts != AV_NOPTS_VALUE) {
                    ...
                    double diff = dpts - ffp_get_master_clock(is);
                    if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD &&
                        diff - is->frame_last_filter_delay < 0 && //TODO 丢帧不生效的问题
                        is->viddec.pkt_serial == is->vidclk.serial &&
                        is->videoq.nb_packets) {
                        is->frame_drops_early++;
                        is->continuous_frame_drops_early++;
...
int ffpipenode_config_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout, IJKFF_Pipenode *node) {
    ...
    case AV_CODEC_ID_H264:
        ...
        switch (opaque->codecpar->profile) {//编解码器特定的, 比特流限制
